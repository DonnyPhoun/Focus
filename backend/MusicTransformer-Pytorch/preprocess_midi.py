...
def prep_maestro_midi(maestro_root, output_dir):
    ...
    midi_filenames = maestro_json.get("midi_filename", {})
    split_types = maestro_json.get("split", {})
    if not midi_filenames or not split_types:
        print("ERROR: Invalid JSON file format. Missing 'midi_filename' or 'split' keys.")
        return False
    
    print("Found", len(midi_filenames), "pieces")
    print("Preprocessing...")
    ...
    # Iterate over the indices, not the dictionaries themselves
    for i in range(len(midi_filenames)):
        midi_filename = midi_filenames.get(str(i))
        split_type = split_types.get(str(i))
        
        # Skip if either key is missing for the current index
        if midi_filename is None or split_type is None:
            print(f"WARNING: Missing data for index {i}. Skipping.")
            continue

        mid_path = os.path.join(maestro_root, midi_filename)
        
        if not os.path.isfile(mid_path):
            print(f"WARNING: Could not find MIDI file at {mid_path}. Skipping.")
            continue
            
        f_name = os.path.basename(midi_filename) + ".pickle"
        ...
        try:
            prepped = midi_processor.encode_midi(mid_path)
            with open(o_file, "wb") as o_stream:
                pickle.dump(prepped, o_stream)
        except Exception as e:
            print(f"ERROR: Failed to process {mid_path} with error: {e}. Skipping.")
            continue

        total_count += 1
        if total_count % 50 == 0:
            print(total_count, "/", len(midi_filenames))
