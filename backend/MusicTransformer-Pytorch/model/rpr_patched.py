...
class TransformerEncoderRPR(Module):
    ...
    # FINAL PATCH: Explicitly accept is_causal and pass it to layers
    def forward(self, src, mask=None, src_key_padding_mask=None, is_causal=False, **kwargs):
        output = src

        for i in range(self.num_layers):
            # Passing is_causal to the layer below
            # NOTE: We MUST pass ALL keyword arguments here to satisfy the chain call in PyTorch 2.x
            output = self.layers[i](output, src_mask=mask,
                                    src_key_padding_mask=src_key_padding_mask, is_causal=is_causal, **kwargs)
...
class TransformerEncoderLayerRPR(Module):
    ...
    # FINAL PATCH: Explicitly accept is_causal and pass it as **kwargs
    def forward(self, src, src_mask=None, src_key_padding_mask=None, is_causal=False, **kwargs):
        # NOTE: We now pass is_causal into the custom MultiheadAttentionRPR function below
        src2 = self.self_attn(src, src, src, attn_mask=src_mask,
                              key_padding_mask=src_key_padding_mask, is_causal=is_causal, **kwargs)[0]
...
class MultiheadAttentionRPR(Module):
    ...
    Modification to add RPR embedding matrix and call custom multi_head_attention_forward_rpr
    ----------
    """
    ...
    # PATCH: Accepting **kwargs to eliminate TypeError
    def forward(self, query, key, value, key_padding_mask=None,
                need_weights=True, attn_mask=None, **kwargs):

        if hasattr(self, '_qkv_same_embed_dim') and self._qkv_same_embed_dim is False:
            # The original code's logic here is commented out and replaced by the rpr-aware version below.
            return multi_head_attention_forward_rpr(
                ...
                v_proj_weight=self.v_proj_weight, rpr_mat=self.Er, **kwargs) # Passed **kwargs
            ...
            return multi_head_attention_forward_rpr(
                ...
                training=self.training,
                key_padding_mask=key_padding_mask, need_weights=need_weights,
                attn_mask=attn_mask, rpr_mat=self.Er, **kwargs) # Passed **kwargs
...
def multi_head_attention_forward_rpr(query,                       # type: Tensor
                                 ...
                                 rpr_mat=None,
                                 # CRITICAL PATCH: Adding **kwargs here prevents TypeError
                                 **kwargs): 
    """
    ----------
    Author: Pytorch
    ...
    if attn_mask is not None:
        ...
        # --- CRITICAL PATCH: Moving mask to q.device for consistency (Fixes mps:0 vs cpu error) ---
